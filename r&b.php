...
<body>
    ...
    <div class="top-wrapper" style="overflow:auto;">
        <div class="container">
    <?php  
         $sql = "CREATE TABLE IF NOT EXISTS rb"
 ." ("
 ...
 .");";
    ...
    if(isset($_POST["Ssubmit"])
        ...
        $pass1=$_POST["pass1"];
         $stmt = $pdo->query($sql);
    $sql = $pdo -> prepare("INSERT INTO rb (name, comment,created, pass) 
    ...
    $sql -> execute();
     ...
     $sql = 'UPDATE rb SET name=:name,comment=:comment,created=:created,pass=:pass WHERE id=:id';
    ...
    if(isset($_POST["delbox"])&&!empty($_POST["pass2"])){
        ...
        $Dnum=$_POST["del"];
            $id=$Dnum;
                $sql = 'SELECT * FROM rb WHERE id=:id ';
        ...
        $results = $stmt->fetchAll(); 
	        foreach ($results as $row){
	            if($pass2==$row['pass']){
	                   $id = $Dnum;
                    $sql = 'delete from rb where id=:id';
                    $stmt = $pdo->prepare($sql);
                    $stmt->bindParam(':id', $id, PDO::PARAM_INT);
                    $stmt->execute();
	            }else{
